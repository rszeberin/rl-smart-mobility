...
class Environment:
    def __init__(self,name, n_edges_max):
        self.graphs = None
        self.name = name
        self.n_edges_max = n_edges_max # I think it is not really needed to add n_edges_max here since we can take it from 
        															# agent
    ...
    def get_reward(self, node, observation=None):
        ...
        if self.name == "MVC":
            ...
            select_node=np.where(self.observation[:].numpy() == 1)[0] #list of selected nodes (partial solution)
            for nodes in adj:
        ...
        # This is our own environment
        elif self.name=="MMCC" :
            done=False
            if len(self.agent.memory) > 0:
                last_node = self.agent.memory[-1][1] #last bust stop (node number)
            else:
                last_node = randint(0, self.graph_init.nodes_count())
    ...
    def get_approx(self):
        ...
        if self.name=="MVC":
            ...
            return (len(self.agent.memory)-1)/n_edges_max #edge cover
